...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=src/saltext/stalekey/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/stalekey/*")
            ...
            #    "--include=src/saltext/stalekey/*,tests/*"
            # )
        finally:
